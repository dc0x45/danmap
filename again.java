...
public class again extends javax.swing.JFrame {
    ...
    private String[] populateList() throws FileNotFoundException{
        ...
        for(int i = 0; i < scansDirLs.length; i++){
            if(scansDirLs[i].getName() != "password.dat");
    ...
    private void initComponents() {
        ...
        jLabel2.setText("IP Address or Hostname to scan:");
        ...
        scanTypeChoice.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Stealth Mode (Takes Longer)",
                "Normal Mode", "No Ping (if you know the server is online for sure)", "Full Comprehensive Scan" }));
    ...
    private void userScansListMouseClicked(java.awt.event.MouseEvent evt) throws FileNotFoundException {
        // TODO add your handling code here:
        if(evt.getClickCount() >= 2){
            String selected = userScansList.getSelectedValue();
            File toDecryptFile = new File("data/" + activeUsername + "/" + selected);
            Scanner toDecryptScanner = new Scanner(toDecryptFile);
            String toDecrypt = toDecryptScanner.nextLine();
            String cipherText = backend.decryptor(toDecrypt, activePassword);
    ...
    private void scanButtonActionPerformed(java.awt.event.ActionEvent evt) {                                           
        ...
        switch(dropdown){
            case "Stealth Mode (Takes Longer)":
                commandString = " -sS ";
                break;
            case "Normal Mode":
                commandString = " ";
        ...
        StringBuilder nmapBuffer = backend.runNmap(commandString);
        rawOutput.setText("Welcome " + activeUsername + "!\n" + nmapBuffer.toString());
        boolean done = backend.saveScanToFile(activeUsername, backend.encryptor(nmapBuffer.toString(), backend.encryptionString(activeUsername, activePassword)));
